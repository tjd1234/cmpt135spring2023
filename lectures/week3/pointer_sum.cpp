...
// - Allocate three new ints on the free store, and store pointers to them in
//   pv.
...
void vector_example2() {
    ...
    // De-allocate the ints pv points to so there's no memory leak. Careful:
    // it's an error to call delete more than once on the same pointer!
    delete p;
    //
...
void vector_example3() {
    ...
    delete pv;
    //
    // DANGER: At this point pv is a dangling pointer. The pointers in pv are no
    //         longer dangling pointers because the vector that contains them
